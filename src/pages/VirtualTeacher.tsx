...
export default function VirtualTeacher() {
  ...
  // États pour Écos avatar
  const [elevenLabsAgent, setElevenLabsAgent] = useState<ElevenLabsAgent | null>(null);
  const [ecosAgent, setEcosAgent] = useState<EcosAgent | null>(null);
  const [ecosSession, setEcosSession] = useState<EcosSession | null>(null);
  ...
  /**
   * Démarrer la conversation avec agent personnalisé + avatar Écos
   */
  const startConversation = useCallback(async () => {
    try {
      ...
      if (stylesError) {
        console.warn('⚠️ Pas de styles d\'apprentissage, utilisation par défaut');
      }
      ...
      if (troublesError) {
        console.warn('⚠️ Pas de troubles détectés');
      }
...
`.trim();
      ...
      console.log('👤 Infos utilisateur:', userInfosFormatted);

      // ========================================
      // ÉTAPE 3 : GÉNÉRER PROMPT PERSONNALISÉ
      // ========================================
      const agentConfig = generateAgentConfig(learningStyles.top3, promptContext);
      console.log('📝 Prompt généré:', agentConfig.systemPrompt.length, 'caractères');
      ...
      // ========================================
      // ÉTAPE 5 : OPTION ÉCOS (DÉSACTIVÉE SI PAS D'ABONNEMENT)
      // ========================================
      // Note: Écos nécessite également un abonnement payant
      // Pour l'instant, on désactive cette fonctionnalité
      // Si vous avez un abonnement Écos, décommentez le code ci-dessous:
      ...
      setEcosAgent(newEcosAgent);
      console.log('✅ Agent Écos créé:', newEcosAgent.id);
      ...
      const newEcosSession = await createEcosSession(
        newEcosAgent.id,
        avatarId
      );

      setEcosSession(newEcosSession);
      console.log('✅ Session Écos créée:', newEcosSession.session_id);
      console.log('🔗 Iframe URL:', newEcosSession.iframe_url);
      */

      console.log('⚠️ Avatar Écos désactivé (nécessite abonnement)');
      ...
      const { data: newConversation, error: conversationError } = await supabase
        ...
        .insert({
          ...
          agent_config: {
            elevenlabs_agent_id: baseAgentId,
            ecos_agent_id: null, // Écos désactivé
      ...
      if (conversationError || !newConversation) {
        throw new Error('Erreur création conversation en base');
      }
      ...
      // ========================================
      // ÉTAPE 7 : DEMANDER PERMISSION MICRO
      // ========================================
      await navigator.mediaDevices.getUserMedia({ audio: true });

      // ========================================
      // ÉTAPE 8 : DÉMARRER SESSION ELEVENLABS AVEC DYNAMIC VARIABLES
      // ========================================
      toast({
        title: 'Connexion vocale...',
        description: 'Activation du microphone',
      });

      const signedUrl = await getElevenLabsSignedUrl(baseAgentId);

      // Préparer les variables dynamiques pour personnaliser l'agent
      ...
      if (ecosSession) {
        await endEcosSession(ecosSession.session_id).catch(console.error);
      }
    }
  }, [
    conversation,
  ...
  /**
   * Arrêter la conversation et l'avatar
   */
  const stopConversation = useCallback(async () => {
    try {
      // 1. Terminer session Écos d'abord
      if (ecosSession) {
        console.log('⏹️ Arrêt session Écos...');
        await endEcosSession(ecosSession.session_id);
        setEcosSession(null);
        setEcosAgent(null);
      }

      // 2. Terminer session ElevenLabs
      console.log('⏹️ Arrêt session ElevenLabs...');
      await conversation.endSession();

      // 3. Reset états
      setElevenLabsAgent(null);

      console.log('✅ Conversation et avatar arrêtés');
    } catch (error) {
      console.error('❌ Erreur arrêt conversation:', error);
    }
  }, [conversation, ecosSession]);
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 pt-20 md:pt-24 pb-24 max-w-6xl">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-3 pt-4">
              <Button
                ...
                disabled={isConnected || isConnecting || isCreatingAvatar}
                size="lg"
                className="w-full"
              >
                <Mic className="h-5 w-5 mr-2" />
                {isCreatingAvatar ? 'Création en cours...' : 'Démarrer la conversation'}
              </Button>
        ...
        {/* Avatar Écos - Professeur Virtuel */}
        {/* Note: Désactivé car nécessite abonnement Écos payant */}
        {/* Pour activer, décommentez le code dans startConversation() */}
        {/*
        <AvatarContainer
        ...
        */}
        ...
        {/* Message informatif pour l'avatar */}
        <Card className="w-full border-dashed border-blue-300 dark:border-blue-700">
          <CardHeader>
            <CardTitle className="text-blue-700 dark:text-blue-400">
              💡 Avatar visuel (fonctionnalité premium)
            </CardTitle>
            <CardDescription>
              L'avatar visuel Écos nécessite un abonnement payant
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Actuellement actif : <strong>Conversation vocale uniquement</strong> (ElevenLabs)
              </p>
              <div className="text-xs text-muted-foreground space-y-1">
                <p>✅ Voix IA professionnelle</p>
                <p>✅ Transcription en temps réel</p>
                <p>✅ Analyse de votre profil d'apprentissage</p>
                <p>⏸️ Avatar visuel (nécessite abonnement Écos)</p>
              </div>
              <p className="text-xs text-blue-600 dark:text-blue-400 mt-3">
                Pour activer l'avatar visuel, souscrivez à Écos et décommentez le code dans VirtualTeacher.tsx
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      {/* Info - Carte désactivée car intégration Écos réussie */}
      {/*
      <Card className="mt-6 border-green-500">
        <CardHeader>
          <CardTitle className="text-green-500">✅ Intégration Écos Avatar</CardTitle>
          <CardDescription>
            Professeur virtuel avec voix et avatar synchronisés
          </CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
          ...
          <ul className="text-sm text-muted-foreground mt-1 space-y-1 list-disc list-inside">
            <li>Agents ElevenLabs dynamiques personnalisés</li>
            <li>Avatar visuel Écos synchronisé avec la voix</li>
            <li>Prompts personnalisés selon le profil d'apprentissage</li>
            <li>Transcription bidirectionnelle en temps réel</li>
            <li>Sauvegarde complète en base de données</li>
          </ul>
        </CardContent>
      </Card>
      */}
